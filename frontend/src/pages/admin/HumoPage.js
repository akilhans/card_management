...
export default function HumoPage() {
  ...
  const fetchUsedCards = async () => {
    try {
      ...
      const humoCards = res.data.filter((c) => c.type === 'HUMO');
      setUsedCards(humoCards);
      
      // Balansni hisoblash
      const balance = humoCards.reduce((sum, card) => sum + (card.receivedAmount || 0), 0);
  ...
  return (
    <div className="p-6">
      ...
      {view === 'used' && (
        <>
          <div className="bg-gradient-to-r from-green-50 to-blue-50 rounded-xl shadow p-6 mb-6 border border-green-200">
                ...
                <tbody className="divide-y divide-gray-100">
                  {filteredUsedCards.map((card, index) => (
                    <tr key={card._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-right font-medium text-green-600">{Number(card.receivedAmount || 0).toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
